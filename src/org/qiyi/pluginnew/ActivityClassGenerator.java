...
public class ActivityClassGenerator {
	...
	public static void createActivityDex(String superClassName, String targetClassName,
		...
		if (saveTo.getName().endsWith(".dex")) {
			Util.writeToFile(dex, saveTo);
		} else {
			JarOutputStream jarOut = new JarOutputStream(new FileOutputStream(saveTo));
	...
	public static <S, D extends S> byte[] createActivityDex(final String superClassName,
		...
		/*
		 * 声明方法：startActivityFromFragment(@NonNull Fragment fragment, Intent intent,
         *  int requestCode, @Nullable Bundle options)
		 */
		declareMethod_startActivityFromFragment(dexMaker, generatedType, superType);
		...
		// declare getOriginalContext
		declareMethod_getOriginalContext(dexMaker, generatedType, superType);
	...
	private static <S, D extends S> void declareFields(DexMaker dexMaker, TypeId<D> generatedType,
		...
		FieldId<D, AssetManager> asm = generatedType.getField(AssetManager, FIELD_ASSERTMANAGER);
		...
		FieldId<D, Boolean> beforeOnCreate = generatedType.getField(TypeId.BOOLEAN,
				FIELD_mOnCreated);
	...
	private static <S, D extends S> void declareMethod_setTheme(DexMaker dexMaker,
			TypeId<D> generatedType, TypeId<S> superType) {
		// Types
		final String methodName = "setTheme";
		MethodId<D, Void> method = generatedType.getMethod(TypeId.VOID, methodName, TypeId.INT);
		...
		MethodId<ActivityOverider, Integer> methodOveride = ActivityOverider.getMethod(TypeId.INT,
				"getPlugActivityTheme", TypeId.get(Activity.class), TypeId.STRING);
		// locals
	...
	private static <S, D extends S> void declareMethod_attachBaseContext(DexMaker dexMaker,
			TypeId<D> generatedType, TypeId<S> superType) {
		// Types
		...
		TypeId<ActivityOverider> ActivityOverider = TypeId.get(ActivityOverider.class);
		TypeId<DisplayMetrics> DisplayMetrics = TypeId.get(DisplayMetrics.class);
		TypeId<Configuration> Configuration = TypeId.get(Configuration.class);

		MethodId<D, Void> method = generatedType.getMethod(TypeId.VOID, "attachBaseContext",
				Context);
		Code methodCode = dexMaker.declare(method, PROTECTED);
		...
		MethodId<Resources, DisplayMetrics> getDisplayMetrics = Resources.getMethod(DisplayMetrics,
				"getDisplayMetrics");
		methodCode.invokeVirtual(getDisplayMetrics, mtrc, superRes);
		//
		// superRes.getConfiguration()
		MethodId<Resources, Configuration> getConfiguration = Resources.getMethod(Configuration,
				"getConfiguration");
		...
		MethodId<Resources, Void> res_constructor = Resources.getConstructor(AssetManager,
		...
		MethodId<S, Void> superMethod = superType.getMethod(TypeId.VOID, "attachBaseContext",
				Context);
		methodCode.invokeSuper(superMethod, null, localThis, newbase);
		methodCode.returnVoid();
	}

	private static <S, D extends S> void declareMethod_getIntent(DexMaker dexMaker,
			TypeId<D> generatedType, TypeId<S> superType) {
		...
		MethodId<D, Intent> method = generatedType.getMethod(Intent, methodName);
		MethodId<S, Intent> superMethod = superType.getMethod(Intent, methodName);
		...
		MethodId<D, ComponentName> getComponent = generatedType.getMethod(ComponentName,
				"getComponentName");

		code.invokeVirtual(getComponent, localComp, localThis);

		MethodId<Intent, Intent> setComponent = Intent.getMethod(Intent, "setComponent",
				ComponentName);
	...
	private static <S, D extends S> void declareMethod_getPackageName(DexMaker dexMaker,
			TypeId<D> generatedType, String pkgName) {
		MethodId<D, String> method = generatedType.getMethod(TypeId.STRING, "getPackageName");
		Code methodCode = dexMaker.declare(method, PROTECTED);
	...
	private static <S, D extends S> void declareMethod_getComponentName(DexMaker dexMaker,
			TypeId<D> generatedType, TypeId<S> superType, String superClassName) {
		TypeId<ComponentName> ComponentName = TypeId.get(ComponentName.class);
		...
		{
			FieldId<D, String> fieldPkg = generatedType.getField(TypeId.STRING, "_pkg");
	...
	private static <S, D extends S> void declareMethod_onCreate(DexMaker dexMaker,
		...
		MethodId<D, Void> method = generatedType.getMethod(TypeId.VOID, "onCreate", Bundle);
		Code methodCode = dexMaker.declare(method, PROTECTED);
		// locals
		Local<D> localThis = methodCode.getThis(generatedType);
		...
		MethodId<ActivityOverider, Void> method_call_onCreate = ActivityOverider.getMethod(
				TypeId.VOID, "callback_onCreate", TypeId.STRING, TypeId.get(Activity.class));
		methodCode.invokeStatic(method_call_onCreate, null, pluginId, localThis);

		// super.onCreate()
		MethodId<S, Void> superMethod = superType.getMethod(TypeId.VOID, "onCreate", Bundle);
	...
	private static <S, D extends S> void declareMethod_getResources(DexMaker dexMaker,
			TypeId<D> generatedType, TypeId<S> superType) {
		TypeId<Resources> Resources = TypeId.get(Resources.class);
		MethodId<D, Resources> getResources = generatedType.getMethod(Resources, "getResources");
		...
		MethodId<S, Resources> superGetResources = superType.getMethod(Resources, "getResources");
	...
	private static <S, D extends S> void declareMethod_getAssets(DexMaker dexMaker,
			TypeId<D> generatedType, TypeId<S> superType) {
		TypeId<AssetManager> AssetManager = TypeId.get(AssetManager.class);
		MethodId<D, AssetManager> getAssets = generatedType.getMethod(AssetManager, "getAssets");
		...
		code.loadConstant(nullV, null);
		FieldId<D, AssetManager> res = generatedType.getField(AssetManager, FIELD_ASSERTMANAGER);
	...
	private static <S, D extends S> void declareMethod_startActivityForResult(DexMaker dexMaker,
			TypeId<D> generatedType, TypeId<S> superType) {
		TypeId<Intent> intent = TypeId.get(Intent.class);
		...
		final boolean isNewSdk = android.os.Build.VERSION.SDK_INT > 15;
		if (isNewSdk) {
			params = new TypeId[] { intent, requestCode, bundle };
		} else {
			params = new TypeId[] { intent, requestCode };
		}
		MethodId<D, Void> method = generatedType.getMethod(TypeId.VOID, methodName, params);
		MethodId<S, Void> superMethod = superType.getMethod(TypeId.VOID, methodName, params);
		Code methodCode = dexMaker.declare(method, PUBLIC);
		TypeId<ActivityOverider> ActivityOverider = TypeId.get(ActivityOverider.class);
		MethodId<ActivityOverider, Intent> methodOveride = ActivityOverider.getMethod(intent,
				"overrideStartActivityForResult", TypeId.get(Activity.class), TypeId.STRING,
		...
		Local<?> args[];
		if (isNewSdk) {
			args = new Local[] { localThis, pluginId, methodCode.getParameter(0, intent)//
	...
	private static <S, D extends S> void declareMethod_startActivityFromFragment(
			DexMaker dexMaker, TypeId<D> generatedType, TypeId<S> superType) {
		TypeId<Fragment> fagment = TypeId.get(Fragment.class);
		TypeId<Intent> intent = TypeId.get(Intent.class);
		TypeId<Integer> requestCode = TypeId.INT;
		TypeId<Bundle> bundle = TypeId.get(Bundle.class);
		TypeId<?>[] params;
		String methodName = "startActivityFromFragment";
		final boolean isNewSdk = android.os.Build.VERSION.SDK_INT > 15;
		if (isNewSdk) {
			params = new TypeId[] {fagment, intent, requestCode, bundle };
		} else {
			params = new TypeId[] {fagment, intent, requestCode };
		}
		MethodId<D, Void> method = generatedType.getMethod(TypeId.VOID, methodName, params);
		MethodId<S, Void> superMethod = superType.getMethod(TypeId.VOID, methodName, params);
		Code methodCode = dexMaker.declare(method, PUBLIC);
		...
		MethodId<ActivityOverider, Intent> methodOveride = ActivityOverider
				.getMethod(intent, "overrideStartActivityFromFragment",
		...
		if (isNewSdk) {
			args = new Local[] { localThis, pluginId//
					, methodCode.getParameter(1, intent)//
					, methodCode.getParameter(2, requestCode) //
					, methodCode.getParameter(3, bundle)
			};
			methodCode.invokeStatic(methodOveride, newIntent, args);
			// super.startActivityFromFragment(...)

			methodCode.invokeSuper(superMethod, null, localThis//
					, methodCode.getParameter(0, fagment)
					, newIntent//
					, methodCode.getParameter(2, requestCode)//
					, methodCode.getParameter(3, bundle) //
					);
	...
	private static <S, D extends S> void declareMethod_onBackPressed(DexMaker dexMaker,
			TypeId<D> generatedType, TypeId<S> superType) {
		TypeId<ActivityOverider> ActivityOverider = TypeId.get(ActivityOverider.class);
		MethodId<D, Void> method = generatedType.getMethod(TypeId.VOID, "onBackPressed");
		Code methodCode = dexMaker.declare(method, PUBLIC);
		// locals -- 一个方法内的本地变量必须提前声明在所有操作之前
		Local<D> localThis = methodCode.getThis(generatedType);
		...
		MethodId<ActivityOverider, Boolean> methodOveride = ActivityOverider.getMethod(
				TypeId.BOOLEAN, "overrideOnbackPressed", TypeId.get(Activity.class), TypeId.STRING);
		methodCode.invokeStatic(methodOveride, localBool, localThis, pluginId);
		// codeBlock: if start
		Label localBool_isInvokeSuper = new Label();
		methodCode.compare(Comparison.EQ, localBool_isInvokeSuper, localBool, localFalse);
		MethodId<S, Void> superMethod = superType.getMethod(TypeId.VOID, "onBackPressed");
		methodCode.invokeSuper(superMethod, null, localThis);
	...
	private static <S, D extends S> void declareMethod_startService(DexMaker dexMaker,
			TypeId<D> generatedType, TypeId<S> superType) {
		TypeId<ActivityOverider> ActivityOverider = TypeId.get(ActivityOverider.class);
		TypeId<ComponentName> returnType = TypeId.get(ComponentName.class);
		TypeId<Intent> Intent = TypeId.get(Intent.class);
		MethodId<D, ComponentName> method = generatedType.getMethod(returnType, "startService",
				Intent);
		MethodId<ActivityOverider, ComponentName> methodOveride = ActivityOverider.getMethod(
				returnType, "overrideStartService", TypeId.get(Activity.class), TypeId.STRING,
				Intent);
		Code methodCode = dexMaker.declare(method, PUBLIC);
		// locals
		Local<D> localThis = methodCode.getThis(generatedType);
	...
	private static <S, D extends S> void declareMethod_getOriginalContext(DexMaker dexMaker,
			TypeId<D> generatedType, TypeId<S> superType) {
		TypeId<ActivityOverider> ActivityOverider = TypeId.get(ActivityOverider.class);
		TypeId<Context> returnType = TypeId.get(Context.class);
		MethodId<D, Context> method = generatedType.getMethod(returnType, "getOriginalContext");
		MethodId<ActivityOverider, Context> methodOveride = ActivityOverider.getMethod(returnType,
				"overrideGetOriginalContext", TypeId.get(Activity.class), TypeId.STRING);
		Code methodCode = dexMaker.declare(method, PUBLIC);
		// locals
		Local<D> localThis = methodCode.getThis(generatedType);
		Local<Context> localComponentName = methodCode.newLocal(returnType);
		Local<String> pluginId = get_pluginId(generatedType, methodCode);

		methodCode.invokeStatic(methodOveride, localComponentName, localThis, pluginId);
		methodCode.returnValue(localComponentName);
	}

	private static <S, D extends S> void declareMethod_bindService(DexMaker dexMaker,
			TypeId<D> generatedType, TypeId<S> superType) {
		...
		MethodId<D, Boolean> method = generatedType.getMethod(returnType, "bindService", Intent,
				Conn, TypeId.INT);
		MethodId<ActivityOverider, Boolean> methodOveride = ActivityOverider.getMethod(returnType,
				"overrideBindService", TypeId.get(Activity.class), TypeId.STRING, Intent, Conn,
				TypeId.INT);
		Code methodCode = dexMaker.declare(method, PUBLIC);
		// locals
		Local<D> localThis = methodCode.getThis(generatedType);
		...
		methodCode.invokeStatic(methodOveride,
				localBool//
				, localThis, pluginId, methodCode.getParameter(0, Intent),
				methodCode.getParameter(1, Conn), methodCode.getParameter(2, TypeId.INT));
		methodCode.returnValue(localBool);
	}

	private static <S, D extends S> void declareMethod_unbindService(DexMaker dexMaker,
			TypeId<D> generatedType, TypeId<S> superType) {
		// void unbindService( conn);
		TypeId<ActivityOverider> ActivityOverider = TypeId.get(ActivityOverider.class);
		TypeId<ServiceConnection> Conn = TypeId.get(ServiceConnection.class);
		MethodId<D, Void> method = generatedType.getMethod(TypeId.VOID, "unbindService", Conn);
		MethodId<ActivityOverider, Void> methodOveride = ActivityOverider.getMethod(TypeId.VOID,
				"overrideUnbindService", TypeId.get(Activity.class), TypeId.STRING, Conn);
		Code methodCode = dexMaker.declare(method, PUBLIC);
		// locals
		Local<D> localThis = methodCode.getThis(generatedType);
		Local<String> pluginId = get_pluginId(generatedType, methodCode);
	...
	private static <S, D extends S> void declareMethod_stopService(DexMaker dexMaker,
			TypeId<D> generatedType, TypeId<S> superType) {
		...
		TypeId<Intent> Intent = TypeId.get(Intent.class);
		//
		MethodId<D, Boolean> method = generatedType.getMethod(returnType, "stopService", Intent);
		MethodId<ActivityOverider, Boolean> methodOveride = ActivityOverider.getMethod(returnType,
				"overrideStopService", TypeId.get(Activity.class), TypeId.STRING, Intent);
		Code methodCode = dexMaker.declare(method, PUBLIC);
		// locals
		Local<D> localThis = methodCode.getThis(generatedType);
		Local<Boolean> localBool = methodCode.newLocal(returnType);
	...
	private static <S, D extends S> void declareLifeCyleMethod(DexMaker dexMaker,
		...
		TypeId<ActivityOverider> ActivityOverider = TypeId.get(ActivityOverider.class);
		MethodId<D, Void> method = generatedType.getMethod(TypeId.VOID, methodName);
		Code methodCode = dexMaker.declare(method, PROTECTED);
		// locals
		Local<D> localThis = methodCode.getThis(generatedType);
		Local<String> pluginId = get_pluginId(generatedType, methodCode);
		...
		MethodId<ActivityOverider, Void> methodOveride = ActivityOverider.getMethod(TypeId.VOID,
				"callback_" + methodName, TypeId.STRING, TypeId.get(Activity.class));
